 ...
 *
 * The turn() function is implemented to demonstrate how to implement vehicle turning.
 * 
...
#define M2_PWM 7    // pwm pin motor
#define M2_INA 8    // control pin INA
#define M2_INB 9    // control pin INB
#define M2_DIAG 10   // diagnose pins (combined DIAGA/ENA and DIAGB/ENB)
#define M2_CS A1     // current sense pin


void setup() {
  // setting up motor connections. Please note the INA and INB pins are switched for motor2 as with this vehicle
  // this motor will run the reverse direction when the vehicle will move forward.

  Motor1.begin(M1_PWM, M1_INA, M1_INB, M1_DIAG, M1_CS);    // Motor 1 object connected through specified pins 
  Motor2.begin(M2_PWM, M2_INB, M2_INA, M2_DIAG, M2_CS);    // Motor 2 object connected through specified pins 
  Serial.begin(115200);   
  analogReference(DEFAULT);
  analogRead(M1_CS); // dummy
  analogRead(M2_CS); // dummy
}
